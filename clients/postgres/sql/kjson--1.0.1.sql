...
-- Unified temporal functions will be defined after kInstant/kDuration types
...
-- Unified temporal functions - kjson temporal types now use kInstant/kDuration directly
-- These functions are deprecated - use kinstant_now(), kDuration constructors, and operators directly

-- Get current instant with nanosecond precision (deprecated - use kinstant_now())
CREATE OR REPLACE FUNCTION kjson_now()
RETURNS kjson
AS $$
SELECT kjson_build_instant(kinstant_now());
$$ LANGUAGE SQL STABLE;

-- Create duration from ISO 8601 string (deprecated - use text::kDuration)
CREATE OR REPLACE FUNCTION kjson_duration(text)
RETURNS kjson
AS $$
SELECT kjson_build_duration($1::kDuration);
$$ LANGUAGE SQL IMMUTABLE STRICT;

-- Add duration to instant (deprecated - use kinstant + kDuration operators)
-- For now, let's use a simpler approach by creating a simple temporal arithmetic function
CREATE OR REPLACE FUNCTION kjson_add_duration(kjson, kjson)
RETURNS kjson
AS $$
SELECT CASE 
    WHEN kjson_typeof($1) = 'instant' AND kjson_typeof($2) = 'duration' THEN
        kjson_build_instant(
            kinstant_from_timestamp(kjson_extract_timestamp($1, VARIADIC '{}'::text[])) + 
            kjson_extract_text($2, VARIADIC '{}'::text[])::kDuration
        )
    ELSE NULL::kjson
END;
$$ LANGUAGE SQL IMMUTABLE STRICT;
...
-- Convenience function - shorter alias for kinstant_now()
CREATE OR REPLACE FUNCTION k_now()
    RETURNS kInstant
    AS $$
SELECT kinstant_now();
...
-- ================================
-- Unified temporal functions - kjson <-> kInstant/kDuration conversion
-- ================================

-- Build kjson instant from kInstant
CREATE OR REPLACE FUNCTION kjson_build_instant(kInstant)
RETURNS kjson
AS 'MODULE_PATHNAME', 'kjson_build_instant'
LANGUAGE C IMMUTABLE STRICT;

-- Build kjson duration from kDuration
CREATE OR REPLACE FUNCTION kjson_build_duration(kDuration)
RETURNS kjson
AS 'MODULE_PATHNAME', 'kjson_build_duration'
LANGUAGE C IMMUTABLE STRICT;

-- Extract kInstant from kjson
CREATE OR REPLACE FUNCTION kjson_extract_kinstant(kjson, VARIADIC text[])
RETURNS kInstant
AS 'MODULE_PATHNAME', 'kjson_extract_kinstant'
LANGUAGE C IMMUTABLE;

-- Extract kDuration from kjson
CREATE OR REPLACE FUNCTION kjson_extract_kduration(kjson, VARIADIC text[])
RETURNS kDuration
AS 'MODULE_PATHNAME', 'kjson_extract_kduration'
LANGUAGE C IMMUTABLE;
